...
test suite:
    ...
    case "{{suite}}" in
    "strategy-withdraw")
        sed -i '' 's#test = ".*"#test = "yarn run ts-mocha -p ./tsconfig.json -t 1000000 tests/07_strategy-tests/16_strategyWithdraw.ts --require tests/hooks/initWasabi.ts"#' Anchor.toml
        ;;
    "strategy-claim")
        sed -i '' 's#test = ".*"#test = "yarn run ts-mocha -p ./tsconfig.json -t 1000000 tests/08_strategy-tests/15_strategyClaim.ts --require tests/hooks/initWasabi.ts"#' Anchor.toml
        ;;
    "setup")
