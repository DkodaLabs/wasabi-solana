...
pub struct CloseShortPositionSetup<'info> {
    ...
    #[account(
      ...
      seeds = [b"short_pool", collateral_vault.mint.as_ref(), owner_currency_account.mint.as_ref()],
      bump,
    )]
...
impl<'info> CloseShortPositionSetup<'info> {
    pub fn validate(ctx: &Context<Self>, args: &CloseShortPositionSetupArgs) -> Result<()> {
...
pub struct CloseShortPositionSetupArgs {
