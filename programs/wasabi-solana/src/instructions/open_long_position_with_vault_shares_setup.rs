...
pub struct OpenLongPositionWithVaultSharesSetup<'info> {
    ...
    // The token account that is the source of the swap (where principal and down payment are sent)
    #[account(mut)]
    ...
    #[account(
        mut,
        seeds = [lp_vault.key().as_ref(), currency.key().as_ref()],
        bump,
...
impl<'info> OpenLongPositionWithVaultSharesSetup<'info> {
    pub fn validate(ctx: &Context<Self>, expiration: i64) -> Result<()> {
        ...
        // Validate TX has only one setup IX and has one following cleanup IX
    ...
    pub fn open_long_position_with_shares_setup(
        &mut self,
        ...
        self.approve_authority_delegation(total_swap_amount)?;

        self.open_position_request.set_inner(OpenPositionRequest {
            min_target_amount,
            max_amount_in: total_swap_amount,
